                ...
                ZStack {
                    VStack {
                        if let currentUser = userManager.currentUser {
                            ...
                            //                .padding(20)
                            VStack {
                                Text("\(EggManager.shared.computedEggPoints)").font(.system(size:40, weight:.bold, design:.rounded)).foregroundStyle(Color("AccentColor"))
                                Text("Points").foregroundStyle(Color("AccentColor"))
                            }.padding(.vertical,10)
                            ...
                            List {
                                ...
                                ForEach(EggManager.shared.boops) { boop in
                                    NavigationLink(destination: EggDetailView(eggId: boop.id, color: String(boop.id.split(separator: "-")[0]), createdAt: boop.createdAt)) {
                                        HStack(spacing: 16) {
                                            Image("egg-token-\(boop.id.split(separator: "-")[0])-svg")
                                                ...
                                                .frame(width:64)
                                            VStack(alignment: .leading, spacing: 0) {
                                                Text(boop.id).font(.system(size:11, design: .rounded)).padding(.leading, 20)
                                            }
                                            Spacer()
